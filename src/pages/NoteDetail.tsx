      ...
      <div className="mx-auto mt-6 w-full max-w-[760px] px-2">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            {note.kind === "daily" ? (
              <h1 className="note-page-title text-2xl md:text-3xl break-words leading-snug">{headerTitle}</h1>
            ) : (
              <Input
                value={title}
                onChange={(e) => { setTitle(e.target.value); save({ title: e.target.value }); }}
                placeholder="Untitled"
                className="note-page-title h-auto border-0 bg-transparent px-0 py-1 text-2xl shadow-none focus-visible:ring-0 md:text-3xl break-words leading-snug"
              />
            )}
          </div>
        </div>
