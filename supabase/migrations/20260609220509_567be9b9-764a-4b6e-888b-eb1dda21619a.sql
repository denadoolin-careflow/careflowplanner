...
BEGIN
  ...
  SELECT cron_secret INTO secret FROM public.changelog_settings WHERE id = true;
  IF secret IS NULL THEN
    secret := replace(gen_random_uuid()::text, '-', '');
    UPDATE public.changelog_settings SET cron_secret = secret WHERE id = true;
  END IF;
  ...
  PERFORM cron.schedule(
    ...
    format($cron$
      select net.http_post(
        url:=%L,
        headers:=jsonb_build_object('Content-Type','application/json','apikey',%L,'Authorization',%L,'x-changelog-cron-secret',%L),
        body:=jsonb_build_object('all', true, 'fromLastPull', true)
      );
    $cron$, fn_url, anon_key, 'Bearer ' || anon_key, secret)
  );
